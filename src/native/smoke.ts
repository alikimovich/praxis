...
export async function runNativeSmoke(host: NativeBridge, fixture: string, root: string) {
  ...
  for (let i = 0; (await host.request('inspectorInspect')).fields < 2 && i < 60; i++) await new Promise(resolve => setTimeout(resolve, 100))
  const editingInspector = await host.request('inspectorInspect')
  if (!editingInspector.visible || editingInspector.fields < 2 || views.has('panel')) throw new Error('Selection must open the native inspector without a panel WebView')
  console.log('Native inspector first-use fields without a property WebView passed.')
