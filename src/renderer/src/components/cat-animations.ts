...
const metadata = import.meta.glob<Animation>(
  '../assets/cat/selected/{idle,think,jump,appear}/animation.json',
  {
    eager: true,
    import: 'default'
  }
)
const sprites = import.meta.glob<string>('../assets/cat/selected/{idle,think,jump,appear}/*.svg', {
  eager: true,
  query: '?url',
  import: 'default'
})
export function catFrames(
  name: 'idle' | 'think' | 'jump' | 'appear'
