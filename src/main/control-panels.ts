 ...
 * live in `<repo>/.dsgn/control-panels.json` — the same sidecar directory as
 ...
 * (the agent is denied `.dsgn/` writes). The file on disk is user-editable and
...
export function saveManifest(
  ...
  return serialize(async () => {
    ...
    if (!store.writable)
      return { error: '.dsgn/control-panels.json is not a valid store — fix or delete it first' }
