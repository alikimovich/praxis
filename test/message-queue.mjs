...
// Landing keeps the queue gated while the completed response can receive Revert.
chat.setActiveChat('landing')
chat.startAssistant('landing')
chat.appendDelta('Finished editing', 'landing')
chat.finish('landing', true)
assert.equal(useChat.getState().byKey.landing.isRunning, true)
assert.equal(useChat.getState().byKey.landing.streamingId, null)
chat.tagRevert('landing', 'landed-group')
assert.equal(useChat.getState().byKey.landing.messages.at(-1).revertGroup, 'landed-group')
let dispatched = 0
queue.add({ key: 'landing', label: 'next', run: async () => { dispatched++ } })
drainMessages(useChat.getState().byKey)
assert.equal(dispatched, 0)
chat.finish('landing')
drainMessages(useChat.getState().byKey)
await settle()
assert.equal(dispatched, 1)
console.log('LANDING QUEUE OK — completed response receives Revert while follow-ups wait')
