...
try {
  ...
  await win.waitForFunction(() => window.__praxisSession.getState().slashCommands.some(s => s.name === 'animation-controls'))
  ...
  await win.waitForFunction(() => window.__praxisSession.getState().slashCommands.some(s => s.name === 'animation-controls'))
  await win.fill('.composer__input', '/animation-con')
  const animationSkill = win.locator('.slash__item').filter({ hasText: 'animation-controls' })
  await animationSkill.waitFor()
  await win.screenshot({ path: 'test/artifacts/animation-controls-skill-menu.png' })
  await animationSkill.click()
  assert.equal(await win.inputValue('.composer__input'), '/animation-controls ')
