...
const noKey = process.env.PRAXIS_TEST_CONTROLS_NO_KEY === '1'
const jev = !noKey && !!(process.env.JEV_AI_GATEWAY_API_KEY || process.env.AI_GATEWAY_API_KEY)
...
let app
try {
 app=await _electron.launch({executablePath:electronPath,args:[join(process.cwd(),'out/main/index.js')],env:{...process.env,...(noKey ? { JEV_AI_GATEWAY_API_KEY: '', AI_GATEWAY_API_KEY: '' } : {}),PRAXIS_USER_DATA:profile,PRAXIS_TEST_SKIP_INTRO:'1'}})
 if(jev) await useSavedJevKey(app)
 await app.evaluate(()=>{const original=globalThis.fetch;globalThis.__jevCalls=0;globalThis.__jevSuccesses=0;globalThis.fetch=async(...args)=>{const response=await original(...args);if(String(args[0]).includes('/v4/ai/evaluation-model')){globalThis.__jevCalls++;if(response.ok)globalThis.__jevSuccesses++}return response}})
 ...
 await win.fill('.composer__input',noKey ? 'Surface controls for the homepage headline.' : `Surface content controls for the homepage headline. First create content.json with headline set to the existing heading, and wire index.html to consume it with refresh on save. Use content_controls catalog then define with one copy section and a required headline text field. ${jev?'Use Jev (engine:jev) to choose the controls, with the original request as prompt.':'Use the current chat model.'} Preserve content and do not add any dependencies or UI to the target. Verify the control tool succeeds.`)
 ...
 if(!existsSync(join(root,'.praxis/content-controls.json')) && /not logged in|unauthorized|usage limit|hit your .*limit/i.test(text)){console.log('CONTENT-CONTROLS-AGENT SKIP — provider unavailable');process.exitCode=0}
   ...
   if(noKey) assert.equal(await app.evaluate(()=>globalThis.__jevCalls),0,'missing key must not make a Jev request')
   ...
   await win.waitForFunction(root=>window.api.contentControls.list(root).then(async panels=>(await window.api.contentControls.get(root,panels[0].id)).value[panels[0].recipe.sections[0].fields[0].key]==='Live agent controls work'),root)
   const previewText = () => app.evaluate(async ({webContents}) => {
     const preview = webContents.getAllWebContents().find(w => /^http:\/\/(127\.0\.0\.1|localhost):\d+/.test(w.getURL()))
     return preview?.executeJavaScript('document.body.textContent')
   })
   for(let attempt=0;attempt<40 && !(await previewText())?.includes('Live agent controls work');attempt++) await new Promise(resolve=>setTimeout(resolve,250))
   assert((await previewText())?.includes('Live agent controls work'),'saving must update the actual page')
