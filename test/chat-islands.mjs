...
try {
  ...
  assert.deepEqual(islands.attachments('resumed-key').map(a => a.turn), [1, 2])
  assert.equal(islands.attachments('resumed-key')[0].view.revision, 2, 'Historical definition stays intact')
  ...
  assert.equal(islands.attachments('resumed-key')[1].view.status, 'unavailable', 'Duplicate success must not revive failed creation')
