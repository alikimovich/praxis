 ...
 *
 *   OK   — a valid manifest landed in the live tree's .dsgn/ with resolving anchors (exit 0)
...
// The turn edits files and .dsgn/ state — run on a throwaway COPY so the real
// fixture stays pristine whatever the agent does.
const project = mkdtempSync(join(tmpdir(), 'dsgn-controls-agent-'))
...
const PROMPT = [
  ...
  'Never create or edit files under `.dsgn/`. Do not ask for confirmation.'
].join('\n')
...
const checkLiveManifest = () => {
  const storeFile = join(project, '.dsgn', 'control-panels.json')
  if (!existsSync(storeFile))
    return { ok: false, why: 'no .dsgn/control-panels.json in the live root' }
  let store
  try {
...
try {
  ...
  for (const args of [
    ['init'],
    ['add', '-A'],
    ['-c', 'user.name=dsgn-test', '-c', 'user.email=test@example.com', 'commit', '-m', 'fixture']
  ]) {
  ...
  const isolation = await win.evaluate(() => window.__dsgnStore.getState().isolation)
